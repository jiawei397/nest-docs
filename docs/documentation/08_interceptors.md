...
例如，考虑一个传入的`POST /cats`请求。此请求将发送到`CatsController`内定义的`create()`处理程序。如果拦截器不调用`next()`，那么`create()`方法将不会被执行，拦截器可以自行修改`Context`实例的响应Body与Status。
...
为了设置拦截器，我们使用从`@nest`包导入的`@UseInterceptors()`装饰器。与Guard、Exception Filter一样，拦截器可以是控制器范围、方法范围或全局范围的。
...
在《[定义Provider](https://www.yuque.com/jiqingyun-begup/yimxc4/cythiv1zfxpmx8hy)》了解更多信息。
:::
...
这个响应是被全局默认的Exception Filter处理后的结果。
...
:::warning
以上仅仅作为示例使用，对于异常的捕获，更推荐使用Exception Filter进行处理。
:::
